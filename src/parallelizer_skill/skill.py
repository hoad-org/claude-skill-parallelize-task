"""Parallelize Task Skill - DevArmor-compliant skill with lifecycle management and event subscriptions."""
...
class ParallelizerSkill:
    """DevArmor-compliant Parallelize Task Skill with lifecycle hooks and event handling.

    This skill manages task parallelization with full DevArmor governance:
    - Lifecycle management (install/upgrade/remove hooks)
    - Event publishing for all parallelization operations
    - Cross-skill communication via event subscriptions
    - Audit trail via DevArmor
    """

    def __init__(
        self, skill_name: str = "parallelize-task", devarmor_api: Optional[DevArmorAPI] = None
    ):
        """Initialize Parallelize Task skill.
    ...
    async def pre_action_check(self, action: str, resource: str, actor: str) -> bool:
        ...
        Args:
            action: Action to perform (e.g., "parallelize_task", "analyze_dag")
            resource: Resource being accessed (e.g., task name)
    ...
    async def publish_event(
        ...
        Args:
            event_type: Type of event (e.g., "task_parallelized", "dag_analyzed")
    ...
    async def publish_task_parallelized(
        self, task_count: int = 0, parallelism_factor: float = 1.0, actor: str = "claude", **details: Any
    ) -> None:
        """Publish event when task is parallelized.

        Args:
            task_count: Number of tasks created
            parallelism_factor: Parallelism factor achieved
            actor: Who parallelized the task
            **details: Additional details
        """
        await self.publish_event(
            event_type="task_parallelized",
            action="parallelize_task",
            resource="task",
            actor=actor,
            details={"task_count": task_count, "parallelism_factor": parallelism_factor, **details},
        )

    async def publish_dag_analyzed(
        self, stage_count: int = 0, actor: str = "claude", **details: Any
    ) -> None:
        """Publish event when DAG is analyzed.

        Args:
            stage_count: Number of stages in DAG
            actor: Who analyzed the DAG
            **details: Additional details
        """
        await self.publish_event(
            event_type="dag_analyzed",
            action="analyze_dag",
            resource="dag",
            actor=actor,
            details={"stage_count": stage_count, **details},
        )

    async def publish_plan_generated(
        self, plan_id: str = "", actor: str = "claude", **details: Any
    ) -> None:
        """Publish event when execution plan is generated.

        Args:
            plan_id: Identifier of the generated plan
            actor: Who generated the plan
            **details: Additional details
        """
        await self.publish_event(
