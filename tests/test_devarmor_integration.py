...
- Event publishing when parallelization operations occur
...
from parallelizer_skill.skill import ParallelizerSkill


class TestParallelizerSkillLifecycle:
    """Test Parallelize Task skill lifecycle hooks."""

    @pytest.mark.asyncio
    async def test_skill_initialization(self):
        """Test skill initialization."""
        skill = ParallelizerSkill(skill_name="parallelize-task")
    ...
    async def test_skill_install_hook(self):
        ...
        skill = ParallelizerSkill(skill_name="test-parallelize")
...
class TestEventPublishing:
    """Test event publishing for parallelization operations."""

    @pytest.mark.asyncio
    async def test_publish_task_parallelized(self):
        """Test publishing task parallelized event."""
        skill = ParallelizerSkill()
        ...
        with patch.object(skill, "devarmor_api") as mock_api:
            ...
            await skill.publish_task_parallelized(
                task_count=8,
            ...
            event = call_args[0][0]
            assert event.action == "parallelize_task"
            assert event.details["task_count"] == 8

    @pytest.mark.asyncio
    async def test_publish_dag_analyzed(self):
        """Test publishing DAG analyzed event."""
        skill = ParallelizerSkill()
        ...
        with patch.object(skill, "devarmor_api") as mock_api:
            ...
            await skill.publish_dag_analyzed(stage_count=3)

            # Verify event was published
            mock_api.event_bus.publish.assert_called_once()
            call_args = mock_api.event_bus.publish.call_args
            event = call_args[0][0]
            assert event.action == "analyze_dag"

    @pytest.mark.asyncio
    async def test_publish_plan_generated(self):
        """Test publishing plan generated event."""
        skill = ParallelizerSkill()
        ...
        with patch.object(skill, "devarmor_api") as mock_api:
            ...
            await skill.publish_plan_generated(plan_id="plan_123")
...
class TestSkillMetadata:
    ...
    def test_skill_name(self):
        """Test skill name."""
        skill = ParallelizerSkill(skill_name="custom-parallelize-skill")
        assert skill.skill_name == "custom-parallelize-skill"
